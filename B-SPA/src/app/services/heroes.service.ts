...
export class HeroesService {
  ...
  constructor() {
   ...
   // Retorna la lista completa de los heroes
   public getHeroes(): Heroe[] {
     return this.Heroes;
    }

    // Retorna la lista completa de los heroes
   public getShareHeroes(texto: string): Heroe[] {
    let heroesEncontrados: Heroe[] = [];
    for ( let heroe of this.Heroes){
      // if (heroe.nombre.toUpperCase() === texto.toUpperCase()) {
      // verifico que si el texto buscado se encuentra dentro del nombre de los heroes usando indexOf
      if ( heroe.nombre.toLowerCase().indexOf( texto.toLowerCase() ) >= 0 ) {
        heroesEncontrados.push(heroe);
        // console.log("cargando heroes");
      }
    }
    ...
    public getHeroe(id: number): Heroe {
      if (id <= 0 && id > this.Heroes.length) {
        const heroeVacio: Heroe = {
          id: -1, nombre : '', bio : '', img : '', aparicion : '', casa : ''
        };
        return heroeVacio;
      }
